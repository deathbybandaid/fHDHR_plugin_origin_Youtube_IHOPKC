...
class HDHRConfig():
    ...
    def config_adjustments_this(self):
        ...
        self.config_adjustments_prayerroom()
    ...
    def config_adjustments_prayerroom(self):
        cache_dir = self.config["main"]["cache_dir"]

        prayerroom_cache = pathlib.Path(cache_dir).joinpath('prayerroom')
        self.config["prayerroom"]["prayerroom_cache"] = prayerroom_cache
        if not prayerroom_cache.is_dir():
            prayerroom_cache.mkdir()
        self.config["prayerroom"]["epg_cache"] = pathlib.Path(prayerroom_cache).joinpath('epg.json')
        prayerroom_web_cache = pathlib.Path(prayerroom_cache).joinpath('prayerroom_web_cache')
        self.config["prayerroom"]["prayerroom_web_cache"] = prayerroom_web_cache
        if not prayerroom_web_cache.is_dir():
            prayerroom_web_cache.mkdir()
        self.config["prayerroom"]["pdf_file"] = pathlib.Path(prayerroom_web_cache).joinpath('sched.pdf')
