...
class Plugin_OBJ():
    ...
    def __init__(self, channels, plugin_utils):
        ...
        web_cache_dir = self.plugin_utils.config.internal["paths"]["cache_dir"]

        self.pdf_sched = pathlib.Path(web_cache_dir).joinpath('sched.pdf')
        ...
        for fhdhr_id in list(self.channels.list[self.plugin_utils.namespace].keys()):
            ...
            for event in events_list:
                ...
                clean_prog_dict = {
                                    ...
                                    "thumbnail": "https://i.ytimg.com/vi/%s/maxresdefault.jpg" % (str(self.origin.video_reference[chan_obj.dict["origin_id"]]["video_id"])),
                                    "title": event['title'],
    ...
    def clear_database_cache(self):
        self.plugin_utils.logger.info("Clearing PDF cache.")
    ...
    def download_pdf_epg(self):
        ...
        if not os.path.exists(self.pdf_sched):
            ...
            self.plugin_utils.logger.info("Checking online PDF for updates.")
            ...
            if not offline_file_time <= online_file_time:
                self.plugin_utils.logger.info("Cached PDF is current.")
            else:
                why_download = "Online PDF is newer."

        if why_download:
            self.clear_database_cache()
            self.plugin_utils.logger.info("%s Downloading the latest PDF..." % why_download)
